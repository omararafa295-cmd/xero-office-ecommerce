...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const paymobCardEnabled = @json($paymobCardEnabled);
        const paymobWalletEnabled = @json($paymobWalletEnabled);
        ...
        function updatePaymentMethodUI() {
            ...
            if (selectedMethod === 'cash') {
                paymentMethodHint.innerHTML = '<span class="text-base">•</span> {{ __('سيتم سداد قيمة الطلب عند الاستلام.') }}';
                return;
            }

            if (selectedMethod === 'card') {
                paymentMethodHint.innerHTML = '<span class="text-base">•</span> {{ __('سيتم استكمال الدفع الإلكتروني بشكل آمن بعد تأكيد الطلب.') }}';
                return;
            }

            paymentMethodHint.innerHTML = '<span class="text-base">•</span> {{ __('سيتم إرسال طلب الدفع إلى محفظتك بعد تأكيد الطلب.') }}';
        }
        ...
        [cardUiNumber, cardUiName, cardUiExpiry, cardUiCvv].forEach((input) => {
