...
<div class="relative z-10 text-center px-4 mt-8">
    ...
    <p class="text-gray-600 dark:text-gray-300 font-bold text-lg md:text-xl max-w-2xl mx-auto mb-10">
        {{ __('أفضل الطابعات، أحبار أصلية، وقطع غيار بضمان حقيقي. كل ما يحتاجه مكتبك في مكان واحد.') }}
    </p>
    
    <a href="#categories" class="inline-flex items-center gap-3 bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-[0_8px_20px_rgba(220,38,38,0.3)] hover:-translate-y-1 text-lg">
        {{ __('تصفح المنتجات') }} <i class="fa-solid fa-arrow-down-long animate-bounce"></i>
    </a>
</div>
    
<div class="relative bg-white dark:bg-gray-800 rounded-[2.5rem] my-16 mx-4 max-w-7xl xl:mx-auto shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col md:flex-row items-center gap-10 p-8 md:p-12">
    ...
    <div class="md:w-1/2 {{ app()->getLocale() == 'ar' ? 'text-right' : 'text-left' }} order-1 md:order-2 relative z-10">
        ...
        <p class="text-gray-600 dark:text-gray-300 font-bold mb-8 text-lg md:text-xl leading-relaxed">
            {{ __('استمتع بأداء لا يضاهى مع أحدث تقنيات الطباعة الليزر. توفير في الحبر، سرعة في الإنجاز، وجودة تدوم طويلاً.') }}
        </p>
        
        <a href="#shop" class="inline-flex items-center gap-3 bg-gray-900 dark:bg-gray-100 hover:bg-red-600 dark:hover:bg-red-600 text-white dark:text-gray-900 hover:text-white px-8 py-4 rounded-xl font-bold transition-colors shadow-md text-lg">
...
<div class="max-w-7xl mx-auto px-4 mb-16 relative z-20" data-aos="fade-up" data-aos-delay="300">
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-x-reverse divide-gray-100 dark:divide-gray-700" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
            <div class="flex flex-col items-center text-center group">
                <div class="w-14 h-14 bg-red-50 dark:bg-gray-700 rounded-full flex items-center justify-center text-red-600 mb-4 group-hover:scale-110 group-hover:bg-red-600 group-hover:text-white transition-all duration-300 shadow-sm">
                    <i class="fa-solid fa-truck-fast text-2xl"></i>
                </div>
                <h4 class="font-black text-gray-900 dark:text-white text-lg mb-1">{{ __('شحن سريع') }}</h4>
                <p class="text-sm font-bold text-gray-500">{{ __('استلم شحنتك خلال 3 أيام عمل') }}</p>
            </div>
            
            <div class="flex flex-col items-center text-center group">
                <div class="w-14 h-14 bg-blue-50 dark:bg-gray-700 rounded-full flex items-center justify-center text-blue-600 mb-4 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shadow-sm">
                    <i class="fa-solid fa-rotate-left text-2xl"></i>
                </div>
                <h4 class="font-black text-gray-900 dark:text-white text-lg mb-1">{{ __('إرجاع مجاني') }}</h4>
                <p class="text-sm font-bold text-gray-500">{{ __('إرجاع مجانى فى حالة التلفيات') }}</p>
            </div>
            
            <div class="flex flex-col items-center text-center group">
                <div class="w-14 h-14 bg-green-50 dark:bg-gray-700 rounded-full flex items-center justify-center text-green-600 mb-4 group-hover:scale-110 group-hover:bg-green-600 group-hover:text-white transition-all duration-300 shadow-sm">
                    <i class="fa-solid fa-headset text-2xl"></i>
                </div>
                <h4 class="font-black text-gray-900 dark:text-white text-lg mb-1">{{ __('خدمة العملاء') }}</h4>
                <p class="text-sm font-bold text-gray-500">{{ __('دعم مجاني على مدار الساعة') }}</p>
            </div>
            
            <div class="flex flex-col items-center text-center group">
                <div class="w-14 h-14 bg-purple-50 dark:bg-gray-700 rounded-full flex items-center justify-center text-purple-600 mb-4 group-hover:scale-110 group-hover:bg-purple-600 group-hover:text-white transition-all duration-300 shadow-sm">
                    <i class="fa-solid fa-credit-card text-2xl"></i>
                </div>
                <h4 class="font-black text-gray-900 dark:text-white text-lg mb-1">{{ __('الدفع') }}</h4>
                <p class="text-sm font-bold text-gray-500">{{ __('طرق دفع تناسب الجميع') }}</p>
            </div>
        </div>
    </div>
</div>
...
<div id="categories" class="max-w-7xl mx-auto px-4 py-12 scroll-mt-20">
    ...
    <h2 class="text-4xl font-black text-gray-900 dark:text-white mb-4 transition-colors">{{ __('تسوق حسب القسم') }}</h2>
    ...
    <div class="flex flex-wrap justify-center gap-6 md:gap-8">
        ...
        <a href="{{ route('category.show', $category->name_en) }}" class="w-40 md:w-48 lg:w-56 group bg-white dark:bg-gray-800 p-6 md:p-8 rounded-[2rem] border-2 border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl hover:border-red-500 dark:hover:border-red-500 transition-all text-center" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
            <div class="w-20 h-20 md:w-24 md:h-24 bg-gray-50 dark:bg-gray-900/50 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-all duration-500 group-hover:scale-110 p-3 relative overflow-hidden">
            ...
            <h3 class="font-black text-base md:text-lg text-gray-900 dark:text-white group-hover:text-red-600 transition-colors">{{ app()->getLocale() == 'ar' ? $category->name_ar : $category->name_en }}</h3>
        </a>
        @endforeach
    </div>
</div>
...
<div class="w-full bg-white dark:bg-gray-800 border-y border-gray-100 dark:border-gray-700 py-8 my-16 overflow-hidden relative shadow-inner">
    ...
    <div class="flex items-center space-x-16 space-x-reverse animate-marquee whitespace-nowrap px-8" dir="ltr">
        @for($i=0; $i<2; $i++)
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">HP</h2>
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">CANON</h2>
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">EPSON</h2>
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">SAMSUNG</h2>
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">BROTHER</h2>
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">LEXMARK</h2>
            <h2 class="text-4xl font-black text-gray-300 dark:text-gray-700 mx-8">XEROX</h2>
        @endfor
    </div>
</div>

<div class="max-w-7xl mx-auto px-4 mb-16">
    <div class="relative bg-gradient-to-r from-red-600 to-red-800 rounded-[3rem] p-10 md:p-16 shadow-2xl flex flex-col md:flex-row items-center justify-between overflow-hidden" data-aos="zoom-in" data-aos-duration="800">
        ...
        <div class="w-full md:w-1/2 z-10 {{ app()->getLocale() == 'ar' ? 'text-right' : 'text-left' }} mb-10 md:mb-0">
            ...
            <h2 class="text-4xl md:text-5xl font-black text-white leading-tight mb-4">{{ __('خصم 30% على أحبار') }}<br>{{ __('الجيل الجديد') }}</h2>
            <p class="text-red-100 font-bold text-lg mb-8">{{ __('العرض ينتهي قريباً، اغتنم الفرصة الآن قبل نفاذ الكمية!') }}</p>
            
            <div id="timer-container" class="flex gap-4 {{ app()->getLocale() == 'ar' ? 'justify-end md:justify-start' : 'justify-start' }}" dir="ltr">
                <div class="bg-white/10 backdrop-blur border border-white/20 rounded-2xl w-20 h-20 flex flex-col items-center justify-center">
                    <span id="hours" class="text-3xl font-black text-white">00</span>
                    <span class="text-xs text-red-200 font-bold mt-1">{{ __('ساعة') }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur border border-white/20 rounded-2xl w-20 h-20 flex flex-col items-center justify-center">
                    <span id="minutes" class="text-3xl font-black text-white">00</span>
                    <span class="text-xs text-red-200 font-bold mt-1">{{ __('دقيقة') }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur border border-white/20 rounded-2xl w-20 h-20 flex flex-col items-center justify-center">
                    <span id="seconds" class="text-3xl font-black text-white">00</span>
                    <span class="text-xs text-red-200 font-bold mt-1">{{ __('ثانية') }}</span>
                </div>
            </div>
        </div>
    ...
    <div id="latest-products-container" class="flex gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth pb-8 hide-scrollbar">
        @foreach($products as $index => $product)
            <div class="flex-none w-[85%] sm:w-[calc(50%-12px)] md:w-[calc(33.333%-16px)] lg:w-[calc(25%-18px)] snap-start bg-white dark:bg-gray-800 rounded-3xl shadow-md border-2 border-gray-200 dark:border-gray-700 hover:border-red-500 dark:hover:border-red-500 overflow-hidden group hover:shadow-[0_10px_30px_rgba(220,38,38,0.15)] transition-all duration-300 relative flex flex-col" data-aos="zoom-in" data-aos-delay="{{ $index * 100 }}">
                ...
                <a href="{{ route('product.show', $product->id) }}" class="relative h-56 overflow-hidden block bg-gray-50 dark:bg-gray-900/80 p-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-center">
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <a href="{{ route('product.show', $product->id) }}">
                        <h3 class="font-bold text-lg text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-red-600 transition-colors">{{ app()->getLocale() == 'ar' ? $product->name_ar : $product->name_en }}</h3>
                    </a>
...
<div id="about" class="max-w-6xl mx-auto px-4 py-12 md:py-16">
    <div class="bg-white dark:bg-gray-800 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700 p-8 md:p-10 relative overflow-hidden">
        ...
        <div class="text-center max-w-4xl mx-auto">
            ...
            <h2 class="text-3xl font-black text-gray-900 dark:text-white mb-5">
                {{ __('من نحن') }}
                <div class="w-12 h-1 bg-red-600 mx-auto mt-3 rounded-full"></div>
            </h2>

            <p class="text-gray-600 dark:text-gray-300 font-bold leading-relaxed mb-8 text-base md:text-lg">
                {{ __('تأسست شركة') }} 
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
            ...
            <div class="bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl flex flex-col items-center text-center border border-gray-100 dark:border-gray-700 transition hover:-translate-y-1 hover:shadow-md">
                <div class="w-10 h-10 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-full flex items-center justify-center mb-3">
                    <i class="fa-solid fa-headset text-lg"></i>
                </div>
                <h3 class="text-xl font-black text-gray-900 dark:text-white mb-1">24/7</h3>
                <p class="text-xs font-bold text-gray-500 dark:text-gray-400">{{ __('دعم فني مستمر') }}</p>
            </div>

            <div class="bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl flex flex-col items-center text-center border border-gray-100 dark:border-gray-700 transition hover:-translate-y-1 hover:shadow-md">
                <div class="w-10 h-10 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-full flex items-center justify-center mb-3">
                    <i class="fa-solid fa-boxes-stacked text-lg"></i>
                </div>
                <h3 class="text-xl font-black text-gray-900 dark:text-white mb-1">500+</h3>
                <p class="text-xs font-bold text-gray-500 dark:text-gray-400">{{ __('منتج متاح') }}</p>
            </div>

            <div class="bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl flex flex-col items-center text-center border border-gray-100 dark:border-gray-700 transition hover:-translate-y-1 hover:shadow-md">
                <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-full flex items-center justify-center mb-3">
                    <i class="fa-solid fa-handshake text-lg"></i>
                </div>
                <h3 class="text-xl font-black text-gray-900 dark:text-white mb-1">1000+</h3>
                <p class="text-xs font-bold text-gray-500 dark:text-gray-400">{{ __('عميل يثق بنا') }}</p>
            </div>

            <div class="bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl flex flex-col items-center text-center border border-gray-100 dark:border-gray-700 transition hover:-translate-y-1 hover:shadow-md">
                <div class="w-10 h-10 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 rounded-full flex items-center justify-center mb-3">
                    <i class="fa-solid fa-award text-lg"></i>
                </div>
                <h3 class="text-xl font-black text-gray-900 dark:text-white mb-1">2004</h3>
                <p class="text-xs font-bold text-gray-500 dark:text-gray-400">{{ __('سنة التأسيس') }}</p>
            </div>
